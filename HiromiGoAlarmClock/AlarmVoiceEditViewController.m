//
//  AlarmVoiceEditViewController.m
//  HiromiGoAlarmClock
//
...
#define MAX_VOICEDATA_NUM 3

@interface AlarmVoiceEditViewController ()

@end

@implementation AlarmVoiceEditViewController

@synthesize managedObjectContext = _managedObjectContext;
@synthesize alarmData = _alarmData;
@synthesize navigationBar;
@synthesize selectedSoundRow;
...
- (void)viewDidLoad
{
    ...
    [_tableData addObjectsFromArray:[VoiceList alarmVoiceSet]];
}

- (void)viewWillAppear:(BOOL)animated {
    
    selectedSoundRow = [self.alarmData.alarmvoice intValue] - 1;
    ...
    self.navigationItem.title = @"アラームボイス選択";
    ...
    for (int i = 0; i < MAX_VOICEDATA_NUM; i++) {
        if (i < 3) {
            soundName = [NSString stringWithFormat:@"alarm0%d" ,i + 1];
        }
//        else {
//            soundName = [NSString stringWithFormat:@"alarm%d" ,i + 1];
//        }
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.alarmData.alarmvoice = [NSNumber numberWithInt:indexPath.row + 1];
